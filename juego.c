...
TDAVec* crear_tablero(int filas, int columnas) {
    ...
    if ((filas*columnas) % 2 != 0) return NULL; 
...
int comparar_cartas(s_Carta* carta_1, s_Carta* carta_2){
    ...
    if (carta_1 == NULL || carta_2 == NULL) return -1;

    if (carta_1 == carta_2) return -1;
    
    if (carta_1->id == carta_2->id) return 1; 
    else return 0;
}
...
void cambiar_turno(int*turno_actual){
    ...
    if(*turno_actual == 1) *turno_actual =2;
    else *turno_actual = 1;
}